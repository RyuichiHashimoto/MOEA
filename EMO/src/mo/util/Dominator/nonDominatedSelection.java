...
/*
	...
	public static Population removeSamePoint(Population d){
	...
	public static Front removeSamePoint(Front d){
		for(int i=0;i<d.size()-1;i++){
			for(int j=i+1;j<d.size();j++){
				if( samePoint(d.get(i),d.get(j)) ){
					d.remove(j);
	...
	public static Population nonDominatedSelectionWithoutSamePoint(Population pop,boolean isMAX){
		return removeSamePoint(nonDominatedSelection(pop, isMAX));
	}
