...
import mo.metaheuristics.MOEAD_CDP.MOEADCDP;
...
public class MOEAD_CDPMain extends AlgorithmMain{

	public MOEAD_CDPMain(Setting test) {
		super(test);
	}
	...
	public void setParameter() throws NameNotFoundException, ClassNotFoundException, JMException {
		...
		problem = MOPFactory.getMOP(Problemname,setting_,"MOEADCDP");

		algorithm = new MOEADCDP(problem);
		...
		if(setting_.getAsStr("ScalarFunction").startsWith("PBI")){
			DirectoryName = "result/MOEADCDP/" + Problemname+"/"+dddname + "/" + setting_.getAsStr("ScalarFunction")+setting_.getAsStr("PBITheta").replace(".","_");
		}else {
			DirectoryName = "result/MOEADCDP/" + Problemname+"/"+dddname + "/" + setting_.getAsStr("ScalarFunction");
		}
		...
		algorithm.setInputParameter("MNP", setting_.getAsDouble("matingNeighborhoodPropability"));
